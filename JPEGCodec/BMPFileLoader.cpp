	...
	for (LONG ypos = ypos_start; ypos < ypos_end; ++ypos) {
		for (LONG xpos = 0; xpos < _infoHeader.biWidth; ++xpos) {
			isOK = true
				...
				&& 0 == fseek(_hFile, (_infoHeader.biBitCount / 8) - sizeof(RGBTriple), SEEK_CUR); //忽略32位深度情况下多余的1字节
			if (!isOK) {
				return false;
			}
		}
		fseek(_hFile, _extraByteCnt, SEEK_CUR); //跳过为补齐4字节而多出的字节
	}
	return isOK;
}
...
bool BMPFileLoader::readRGB(Matrix<RGBTriple>* rgbMatrix) {
	...
	for (LONG ypos = 0; ypos < _infoHeader.biHeight; ++ypos) {
		...
		isOK = true
			...
			&& 0 == fseek(_hFile, _extraByteCnt, SEEK_CUR); //跳过为补齐4字节而多出的字节
		if (!isOK) {
			return false;
		}
	}
